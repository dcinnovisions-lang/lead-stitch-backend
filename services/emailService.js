...
class EmailService {
  ...
  getBaseUrl() {
    ...
    const finalUrl = appUrl || 'http://localhost:5000';
  ...
  async logTrackingEvent({ campaignId, recipientId, eventType, eventData = {} }) {
    ...
    try {
      if (this.trackingEventsHasCampaignColumn !== false) {
      ...
      if (error.name === 'SequelizeDatabaseError' && error.message.includes('campaign_id')) {
        this.trackingEventsHasCampaignColumn = false;
        try {
          await EmailTrackingEvents.create({
            campaign_recipient_id: recipientId,
            event_type: eventType,
