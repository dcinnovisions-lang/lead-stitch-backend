...
class ScrapingNotificationService {
    ...
    async initializeTransporter() {
        ...
        try {
            ...
            console.error('❌ [Email Notification] Error initializing email transporter:', error.message);
            console.error('❌ [Email Notification] Error details:', error);
            if (error.response) {
                console.error('❌ [Email Notification] SMTP Response:', error.response);
            }
            if (error.responseCode) {
                console.error('❌ [Email Notification] SMTP Response Code:', error.responseCode);
            }
            console.error('❌ [Email Notification] Email notifications will be disabled until SMTP is properly configured.');
            this.transporter = null;
            this.initialized = true;
        }
    }
    ...
    async sendScrapingCompletionNotification(requirementId, profilesCount, success = true, errorMessage = null) {
        ...
        try {
            ...
            if (success) {
                ...
                console.log(`📧 [Email Notification] Sending success email to: ${userEmail}`);
                ...
                console.log(`✅ [Email Notification] Success email sent successfully!`);
                ...
                console.log(`✅ [Email Notification] Accepted: ${JSON.stringify(mailResult.accepted)}`);
                if (mailResult.rejected && mailResult.rejected.length > 0) {
                    console.warn(`⚠️ [Email Notification] Rejected: ${JSON.stringify(mailResult.rejected)}`);
                }
