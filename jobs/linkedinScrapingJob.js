...
async function processLinkedInScraping(job) {
    ...
    try {
        ...
        console.log(`[Apollo Job] 🔍 Transformed profiles ready for storage: ${profilesToStore}`);
        ...
        console.log(`[Apollo Job] ✅ Job complete! Profiles: ${storedProfiles.length}, Emails found: ${emailFoundCount}`);

        // Send email notification after successful scraping
        try {
            ...
            if (emailSent) {
                console.log(`[Apollo Job] ✅ Email notification sent successfully`);
            } else {
                console.warn(`[Apollo Job] ⚠️  Email notification failed to send`);
            }
        } catch (emailError) {
        ...
        // Send email notification for failed scraping
        try {
            const emailSent = await scrapingNotificationService.sendScrapingCompletionNotification(
                requirementId,
                0,
            ...
            if (emailSent) {
                console.log(`[Apollo Job] ✅ Failure email notification sent successfully`);
            } else {
                console.warn(`[Apollo Job] ⚠️  Failure email notification failed to send`);
            }
        } catch (emailError) {
